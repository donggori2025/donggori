...
- **RLS:** 애플리케이션 테이블은 lockdown migration 적용 후 브라우저 역할(`anon`, `authenticated`)의 직접 접근을 차단한다. 서버 API만 `SUPABASE_SERVICE_ROLE_KEY`로 접근한다.
...
| `kakao_message_consent` | boolean | | 레거시 컬럼 (자동 알림톡/SMS 미제공, 앱에서 미사용) |
...
## 레거시 테이블: `message_logs`

과거 알림톡/SMS 발송 이력용이다. 현재 자동 발송 기능은 제공하지 않으며, 백업·보존 정책을 정한 뒤 별도 변경으로 제거 검토한다.
...
## 레거시 테이블: `factory_auth`

과거 공장 사장님 로그인용이다. 공장 로그인·공장 대시보드·`factory_session`은 제거됐으며, 새 환경에 생성하지 않는다. 기존 테이블은 백업 후 별도 변경으로 제거 검토한다.
...
- 과거 `User`, `FactoryAuth`, `PhoneOtp` 모델 참고용이며 런타임에는 사용하지 않는다.
...
7. `supabase/migrations/20260828_email_otp_verify_attempts.sql` (email_otps.verify_attempts 및 음수 방지 제약)
8. `supabase/migrations/20260828_notice_visibility.sql` (공지 공개 상태·기간)
9. scripts/seed_default_popups.sql (선택)

애플리케이션 코드 배포와 핵심 흐름 검증이 끝난 뒤에만 `supabase/migrations/20260828_lock_down_app_tables.sql`을 적용한다. 자세한 순서는 [hardening 데이터베이스·배포 문서](../hardening/database-and-deployment.md)를 따른다.
```
...
- `factory_auth.factory_id` — 레거시 테이블의 UNIQUE 제약 (새 기능에서 미사용)
