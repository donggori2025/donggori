    ...
    <AuthShell>
      ...
      <div className="mb-8 flex flex-col items-center">
        ...
        <div className="text-gray-500 text-sm">
          이미 계정이 있으신가요? <Link href="/sign-in" className="font-semibold text-black underline underline-offset-4">로그인</Link>
        </div>
      </div>
      {/* 회원가입 폼 */}
      <form onSubmit={handleSubmit} className="flex w-full max-w-xl flex-col gap-4 rounded-lg border border-dg-line bg-white p-6 sm:p-8">
        {/* 이름 입력 */}
        ...
        <div className="flex gap-2">
          ...
          />
          {emailVerified ? (
            <span className="px-4 py-2 bg-green-100 text-green-700 rounded-lg text-sm font-semibold">인증완료</span>
          ) : verificationSent ? (
            <button type="button" className="px-4 py-2 bg-gray-200 rounded-lg text-sm font-semibold" disabled>
              인증코드 발송됨
            </button>
          ) : (
            <button type="button" onClick={requestEmailOtp} className="px-4 py-2 bg-gray-200 rounded-lg text-sm font-semibold hover:bg-gray-300" disabled={loading || !email}>
              이메일 인증
            </button>
          )}
        </div>
        ...
        {verificationSent && !emailVerified && (
          <div className="flex gap-2 items-center">
            <input
              ...
              className="border rounded px-3 py-2 flex-1"
            />
            <button type="button" onClick={handleVerifyCode} className="rounded-lg bg-black px-4 py-2 text-sm font-semibold text-white hover:bg-gray-800" disabled={loading}>
              인증 확인
            </button>
            {/* 타이머/재요청 */}
            {!canResend ? (
              ...
              <button type="button" onClick={handleResend} className="w-16 text-center text-xs text-black underline" disabled={loading}>
                인증번호 재요청
              </button>
            )}
          </div>
        ...
        {error && (
          <div className="text-red-500 text-sm text-center">
            ...
            {error.includes("이미 가입된 이메일 주소입니다") && (
              <>
                {" "}
              <Link href="/sign-in" className="text-black underline">로그인 하러가기</Link>
              </>
            )}
          </div>
        )}
        <button
          type="submit"
          className="w-full bg-black text-white py-3 rounded-lg font-bold text-lg mt-2 hover:bg-gray-900 transition disabled:bg-gray-200 disabled:text-gray-400 disabled:cursor-not-allowed"
          disabled={
            loading ||
            !name.trim() ||
            !email ||
