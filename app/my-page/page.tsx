...
export default function MyPage() {
  ...
  if (!mounted) {
    return <div className="mx-auto mt-20 max-w-md rounded-lg border border-dg-line bg-white p-8 text-center">로딩 중...</div>;
  }

  if (!authUser && !naverUser && !kakaoUser) {
    return <div className="mx-auto mt-20 max-w-md rounded-lg border border-dg-line bg-white p-8 text-center">로그인 후 이용 가능합니다.</div>;
  }
  ...
  function FactoryRequestCard({ req }: { req: MatchRequest }) {
    ...
    if (isDesignRequest) {
      ...
      return (
        <div className="rounded-lg border border-dg-line bg-white p-4 transition-colors duration-200 hover:bg-gray-50 md:p-6">
  ...
  return (
    <div className="max-w-[1400px] mx-auto py-4 md:py-16 px-4 h-full min-h-[500px]">
      ...
      <div className="hidden md:flex flex-row gap-8 h-full min-h-0">
        {/* 왼쪽 사이드바: 메뉴만 */}
        <aside className="flex h-full min-h-0 w-1/4 min-w-[220px] flex-col self-stretch rounded-lg border border-dg-line bg-white p-6">
        ...
        <section className="min-h-0 flex-1 rounded-lg border border-dg-line bg-white p-8">
          {selectedMenu === "프로필" && (
            <div>
