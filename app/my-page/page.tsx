...
function PasswordChangeSection() {
  ...
  const handleSubmit = async (event: FormEvent<HTMLFormElement>) => {
    ...
    try {
      ...
      window.setTimeout(() => window.location.assign("/sign-in?message=password_changed"), 800);
    } catch (error) {
  ...
  return (
    <form onSubmit={handleSubmit} className="mt-4 max-w-lg space-y-4 rounded-xl border border-gray-200 bg-gray-50 p-5">
      <label className="block text-sm font-medium text-gray-700">
        ...
        <input
          ...
          className="mt-1 w-full rounded-lg border border-gray-300 bg-white px-3 py-2 outline-none focus:border-violet-500"
        />
      </label>
      <label className="block text-sm font-medium text-gray-700">
        새 비밀번호 (10자 이상)
      ...
      {message && <p className={`text-sm ${isError ? "text-red-600" : "text-emerald-700"}`}>{message}</p>}
          ...
          <section className="rounded-2xl border border-gray-200 bg-white p-6 md:p-8">
            ...
            {user.signupMethod === "email" && (
              <div className="mt-10 border-t border-gray-100 pt-7">
                <h3 className="mb-2 text-base font-bold text-gray-900">비밀번호</h3>
                <p className="mb-4 text-sm text-gray-500">변경하면 모든 기기에서 로그아웃됩니다.</p>
