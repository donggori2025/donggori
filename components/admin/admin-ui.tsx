...
export function AdminCard({
  ...
  return (
    <div className={`rounded-lg border border-dg-line bg-white ${className}`}>
      {(title || description) && (
...
export function AdminButton({
  ...
  return (
    <button
      className={`inline-flex items-center justify-center gap-2 rounded-lg px-4 py-2.5 text-sm font-medium transition disabled:cursor-not-allowed ${buttonStyles[variant]} ${className}`}
      {...props}
    >
      {children}
    </button>
...
export function AdminAlert({
  ...
  return (
    <div className={`mb-6 rounded-lg border px-4 py-3 text-sm ${styles[type]}`}>{children}</div>
  );
}
