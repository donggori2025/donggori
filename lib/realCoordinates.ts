...
// 인덱스로 실제 좌표 가져오기
export function getRealCoordinateByIndex(index: number): { lat: number; lng: number } | null {
  if (index >= 0 && index < realCoordinates.length) {
...
export function getDongdaemunCenter(): { lat: number; lng: number } {
  const validCoords = realCoordinates.filter(coord => 
    coord.lat >= 37.56 && coord.lat <= 37.61 && 
    coord.lng >= 127.02 && coord.lng <= 127.08
  );
  
  if (validCoords.length === 0) {
    return { lat: 37.5740, lng: 127.0350 }; // 기본 동대문구 중심
  }
