...
// 서버 라우트에서 RLS를 우회해 생성할 때는 API route + getServiceSupabase를 사용하세요.
export async function createUserWithServiceRole(_userData: CreateUserData): Promise<User> {
  throw new Error('createUserWithServiceRole는 서버 API에서만 사용할 수 있습니다.');
}
